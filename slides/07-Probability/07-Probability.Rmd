...
<TODO: say what probability and inference are. check out monogan and whitten & kellstedt>
...
```{r polling histogram 2, echo = FALSE, out.width = '45%'}
tibble(results) %>% 
  ggplot() +
  geom_histogram(aes(x=results), color = 'black', binwidth = 5) +
  geom_vline(xintercept = 526, color = 'red') +
...
That's called the **sampling distribution**, the PDF you get if you repeatedly sample from a distribution and recompute the test statistic each time.
...
```{r wealth and support for reducing mortgage interest deduction}
n1 <- 500
n2 <- 500
...
```{r compute difference in proportions}
