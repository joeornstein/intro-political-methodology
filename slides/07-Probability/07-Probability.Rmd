...
**Our Approach**: Take a fair coin, flip it an enormous number of times, and see how often you get 65 heads in 100 flips.
...
Create a function called `draw_n_letters()`, which draws $N$ letters at random from the alphabet.

--

```{r create draw letters function}
...
Create a function called `fraction_Q()`, which draws $N$ letters from the alphabet at random and returns the fraction that are Q's.
...
```{r compute p-value}
p <- sum(results >= 65) / length(results)
...
2. Compute the distribution of the test statistic under $H_0$

3. Compare the test statistic from your observed data with the null distribution.
...
The sum (or mean) of a large number of independent random variables will be normally distributed.

---

Prove it

<TODO: don't pull from a r function. pull from **data**>

```{r load CCES}
CCES <- read_rds('data/CCES_2018.RDS')

ggplot(CCES) + 
  geom_histogram(aes(x=2018-birthyr))
```

---

```{r sample age}
# draw a sample of age
sample_ages <- function(data, n){
  data %>% 
...
sample_ages(data = CCES, n = 100)

mean(sample_ages(data = CCES, n = 100))
```

---

```{r repeat mean sample ages}
replicate(10000, mean(sample_ages(data = CCES, n = 100))) %>% 
  hist
```
...
sum_of_random_variables <- function(n,m){
  replicate(n, sum_rv(m))
}

sum_of_random_variables(n=100000, m = 50) %>% hist
...
num_draws <- 100000 # draw 100,000 random Bernoulli draws
p <- 0.4 # p = 0.4
