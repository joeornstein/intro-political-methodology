...
Here's a function that will flip a coin $N$ times and return the fraction that landed on heads. (**Note**: it calls the `flip_coin()` function we just made!)
...
Create a function called `count_Q()`, which draws $N$ letters from the alphabet at random and counts the number of Q's.
...
```{r replicate}
# Flip a coin 100 times and count the fraction of heads. 
# Repeat that 100,000 times and save the results
results <- replicate(100000, fraction_heads(N = 100))
```
...
tibble(results) %>% 
  ...
  labs(x = 'Fraction Heads in 100 Fair Coin Flips',
       y = 'Count')
```
...
By the way, that's called the **binomial** distribution. Sum of a bunch of Bernoulli random variables. 
...
If repeatedly draw a random variable a **large number of times**, the average draw will converge on the expected value.
...
**Example:** If you flip a coin 2 million times, roughly how many will be heads?
