---
title: "Correlation, Linear Models, and Matrix Algebra (Part 1 of 2)"
...
```{r estimate slope}
slope_estimate <- sum(data$X * data$Y) / sum(data$X^2)

linear_model_fit <- lm(Y~X, data = data) 
...
1. Null Hypothesis: b = 0

2. Generate Sampling Distribution if b = 0

3. Compare observed value to sampling distribution

---

## Where do we get the sampling distribution?

When we do regression, the random component is that noise term $\varepsilon$. Imagine that we're repeatedly drawing a new $\varepsilon$ vector with each sample.

```{r sampling distribution}

get_slope_estimate <- function(a = 0, b = 0, X){
  
  # Create data assuming null hypothesis (b = 0, a = 0)
  n <- length(X)
...
```{r sampling distribution}
sampling_distribution <- replicate(10000, get_slope_estimate(X = data$X))
