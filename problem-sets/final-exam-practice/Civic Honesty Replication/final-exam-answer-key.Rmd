...
Does the result hold with matching?
...
How strongly does civic honesty correlate with GDP per capita?

```{r gdp per capita}
library(janitor)

gdp <- read_csv('data/cleaned_WB.csv') %>% 
  clean_names() %>% 
...
return_rates <- data %>% 
  filter(cond == 1) %>% 
  group_by(Country) %>% 
  summarize(return_rate = mean(response)) %>% 
  left_join(gdp, by = 'Country')

ggplot(return_rates) + 
  geom_text(mapping = aes(x=gdp_per_capita, y = return_rate,
                          label = Country), size = 2) +
  geom_smooth(mapping = aes(x=gdp_per_capita, y = return_rate),
              method = 'lm') + 
  scale_x_log10() + 
  theme_bw() +
  labs(x = 'GDP Per Capita', y = 'Wallet Return Rate')

gdp_lm <- lm(return_rate ~ log2(gdp_per_capita), data = return_rates)
summary(gdp_lm) # doubling gdp per capita on average yields a 14pp increase in return rates
```
