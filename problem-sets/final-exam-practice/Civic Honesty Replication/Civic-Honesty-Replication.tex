...
\StringTok{  }\KeywordTok{filter}\NormalTok{(Country }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{'Poland'}\NormalTok{, }\StringTok{'UK'}\NormalTok{, }\StringTok{'USA'}\NormalTok{),}
\NormalTok{         cond }\OperatorTok{%in%}\StringTok{ }\DecValTok{0}\OperatorTok{:}\DecValTok{2}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
The following code chunk outputs a pretty table like the one in Table S8
in the original paper's Supplementary Materials. I provide it to you
free of charge. All you need to do is make sure that \texttt{lm1} and
\texttt{lm2} are correctly specified.
