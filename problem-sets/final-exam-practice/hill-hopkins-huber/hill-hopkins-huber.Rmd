...
The following code chunk outputs a pretty table like the one in Table 1 of . I provide it to you free of charge. All you need to do is modify the first line so that it includes your models.

```{r model table, results = 'asis', echo=FALSE}
lm1 <- lm(response ~ age + male, data = data)
lm2 <- lm(response ~ age, data = data)

library(stargazer)
stargazer(lm1,lm2,
          title = 'Change in Republican vote share 2012 to 2016 and change in Hispanic population, various time intervals',
          omit = 'Constant', # omits the y-intercept term
