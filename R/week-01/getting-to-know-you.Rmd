...
On average, we are `r (d$height_inches %>% na.omit %>% mean / 12) %>% floor` feet, `r (d$height_inches %>% na.omit %>% mean %>% floor) %% 12` inches tall. `r sum(d$cats_or_dogs == 'Dogs') / nrow(d) * 100 %>% floor`% of us prefer dogs over cats. `r sum(d$beach_or_mountains == 'Beach') / nrow(d) * 100 %>% floor`% of us prefer beaches over mountains. Here is one way to visualize all that information.
