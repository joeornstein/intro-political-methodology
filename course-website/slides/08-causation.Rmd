...
Best we can do is estimate the **average treatment effect**.
...
Task of causal inference: systematically eliminating alternative explanations until the only credible one remaining is that $X$ causes $Y$.
...
Unless you condition on illness, it seems like hospitals cause death...
...
Using `daggity` or pen-and-paper, identify which variable(s) you can condition on to recover the true causal effect of $X$ on $Y$. Then estimate it in `R`. Compare your estimate with the slope you get from `lm(Y~X)`.
...
Broadly speaking, there are two ways you can make causal claims. 

1. Close all the back doors between your treatment and outcome. 
